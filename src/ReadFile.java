    ...
    public static void main(String[] args) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("secret.txt"))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] cipherBlocks = line.split(" ");
                for (String cipherBlock : cipherBlocks) {

                    cipherTextList.add(cipherBlock);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        for (String cipherBlock : cipherTextList) {
            System.out.println("word : " + cipherBlock);
            String decryptedBlock = applyDecryption(cipherBlock, key);
            decryptedTextList.add(decryptedBlock);
        }

        try (FileWriter writer = new FileWriter("plain.txt")) {
            for (String decryptedBlock : decryptedTextList) {
                writer.write(hexToAscii(decryptedBlock));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }

        System.out.println("Decryption completed. Check 'plain.txt' for the decrypted text.");
    }

public static String hexToAscii(String hex) {
    hex = hex.replaceAll("00", ""); // Remove all occurrences of "00" in the hex string.
...
//        String binaryText = hexToBinary(text);
    ...
    private static String applyDecryption(String cipherText, String key) {
        ...
        System.out.println(round1Decipher);
        String round2Decipher = decipher((round1Decipher), roundKey1, 2);
        System.out.println(" the word deciphered is " + hexToAscii(round2Decipher));
    ...
    private static String decipher(String cipherText, String key, int roundCount) {
        ...
        decryptedText =  binaryToHex(binaryXOR( hexToBinary(key), hexToBinary(decryptedText))); // convert hex to binary and return binary to hex result
